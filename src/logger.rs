...
impl Destination {
    ...
    pub fn isatty(&self) -> bool {
        match *self {
            Destination::Stdout => atty::is(atty::Stream::Stdout),
            Destination::Stderr => atty::is(atty::Stream::Stderr),
        }
    }
}
...
impl Logger {
    ...
    pub fn new(destination: Destination, level: LevelFilter, theme: Theme) -> Logger {
        Logger {
            destination,
            level,
...
impl Theme {
    ...
    pub fn paint_log_level(&self, level: Level) -> ANSIGenericString<'static, str> {
        let (style, name) = match level {
            Level::Error => (self.error, "😡 E "),
            Level::Warn => (self.warn, "😥 W "),
            Level::Info => (self.info, "😋 I "),
            Level::Debug => (self.debug, "🤔 D "),
            Level::Trace => (self.trace, "🤓 T "),
        };
        style.paint(name)
    }
}
